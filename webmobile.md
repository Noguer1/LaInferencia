...
> - **Fase 3** ✅ cerrada. La portada móvil ya estaba adaptada (hero + temas, sin columnas de escritorio). Se añadió una tarjeta del Artículo de la Semana (`#mob-weekly-home`) tras la rejilla de temas, que abre el artículo en su modal. El bloque de Fuera de Bata en la home se descartó a propósito: la sección ya tiene su pestaña y su página, meterlo en la home lo diluye.
> - **Fase 4** ✅ hecha. `@view-transition` entre documentos: hecho. Menú hamburguesa: ya era un panel lateral desde la derecha. El **glosario** pasa de ventana centrada a **panel lateral desde la derecha** en móvil (consistente con el hamburguesa). El **selector de tema** se deja como está (ya es un popup anclado a su botón arriba a la derecha; convertirlo en panel para 7 muestras de color sería peor).
> - **Fase 5** ✅ hecha (solo el formato app, sin lectura offline por decisión). `manifest.webmanifest` (standalone, iconos 192/512, shortcuts a Fuera de Bata y Explorar), metas de Apple, y un `sw.js` mínimo (passthrough, no cachea contenido) para que la web sea instalable en Chrome/Android. Sin prompt propio de instalación: se usa el "Añadir a pantalla de inicio" del navegador.
