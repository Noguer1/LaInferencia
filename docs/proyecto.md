...
- **`.sidebar-fdb-card`**: HTML estático (sin JS de renderizado), vende Fuera de Bata con un resumen corto y el botón `#sidebar-fdb-btn`. Desde septiembre 2026 su listener (IIFE en `main.js`) navega directo a la página estática `/fuera-de-bata/`, ya no abre el panel SPA.

**Fuera de Bata movido a páginas propias (septiembre 2026).**

Toda la sección son páginas HTML **hechas a mano** en `/fuera-de-bata/`. `generate-pages.js` NO las genera ni las sobreescribe en deploy (ojo: hubo un WIP "Operación Fénix E4" con `buildBataArticlePage()` para generar las piezas desde `BATA_ARTICLES`, pero se revirtió porque rompía el build; si en el futuro entran muchas piezas de colaboradores conviene retomarlo).

- `/fuera-de-bata/index.html` — landing de la sección. Rediseñada a fondo (septiembre 2026): `.fdb-hero` con H1 "Fuera de Bata" (titular abierto, no encajonado) + `.fdb-featured` (tarjeta destacada, hoy la pieza de Miguel sobre análisis del lenguaje); `.fdb-catalog-head` con filtros de área (`.fdb-filter`) + buscador por autor (`#fdb-author-input`); `.fdb-index` rejilla 2 col con `.fdb-index-wide` para "lo más reciente"; numeración editorial 01–10 por contador CSS. Cierre con `.fdb-callout` "Lo que ganas publicando en Fuera de Bata" (grid 2 col: pitch + lista de beneficios). Paleta SOLO azul/cyan de la web, degradados sutiles con `color-mix`, sin sistema de color por categoría. Todo el CSS de la sección vive en un `<style>` en el propio archivo (prefijo `.fdb-`).
- **Modal de captación** (`#fdb-modal`): los dos CTA de la landing ("Cómo publicar aquí" del hero y "Quiero publicar aquí" del callout, clase `.js-fdb-open`) abren un modal con fondo desenfocado en vez de navegar. Contenido: cómo funciona (3 pasos), qué te llevas (5 beneficios), nota de selección editorial, y el correo `contacto@lainferencia.com` como texto (no enlace). En móvil ≤560px es sheet inferior. Focus trap, cierre con Esc / clic fuera / X. Sin JS cae al `mailto:` prerrellenado del `href`. Sustituyó a la antigua página `/fuera-de-bata/colaborar/`, que se eliminó.
- `/fuera-de-bata/<slug>/` — 10 piezas (3 de Miguel `AUTHOR_OWN_WORKS` + 7 de colaboradoras), también a mano. Mismo navbar estático que la landing (marca + enlaces Fuera de Bata / Biblioteca + `#theme-toggle` + "Ir a La Inferencia"), movido por `js/theme-static.js`.

El nombre interno "El Claustro" NO aparece en la web. En la SPA el panel `#panel-repositorio` está vaciado a un teaser con botón "Entrar en Fuera de Bata" → `/fuera-de-bata/`; el enlace del footer navega directo. La tarjeta del sidebar izquierdo (`#sidebar-fdb-btn`) navega directo a `/fuera-de-bata/` **en escritorio**; en móvil (pendiente de rediseño) sigue abriendo la vista in-app, que ya lleva su propio botón para entrar. La pestaña oculta `#tab-repositorio` y su JS (`initBataSecciones`, `renderBataFull`, research toggle, ramas `type === 'doc'|'docFeatured'` del buscador) quedan inertes (guards de nulos).

**Barra de progreso de lectura en páginas estáticas (septiembre 2026).** La línea azul→cyan bajo la navbar que crece con el scroll, que ya tenía la SPA (lógica en `main.js`), ahora está en todas las páginas estáticas: markup en `staticHero()` + `<script defer src="/js/reading-progress.js">` en `staticFooterScripts()` de `generate-pages.js` (cubre artículos, categorías, guías, biblioteca, autor), y a mano en la landing y las 10 piezas de Fuera de Bata. `js/reading-progress.js` es autocontenido (scroll de `window`, `requestAnimationFrame`, `transform: scaleX()`). Posición: `body:not(:has(#app)) .reading-progress-bar { top: 60px }` en `styles.css`, porque la navbar estática mide 60px y no 64/52 como la de la SPA.
...
**Rutas de Aprendizaje retiradas (septiembre 2026).** Eran playlists curadas de artículos que ya existen en sus categorías, no aportaban valor propio. Fuera de: nav de la SPA y de las páginas estáticas, footer, widget "Rutas completadas" de "Tu Progreso" (SPA y sección "Yo" móvil), sitemap, índice de búsqueda y el banner `.ruta-banner` / `.ruta-finish` que `generate-pages.js` inyectaba en 16 artículos. En `generate-pages.js` el array `RUTAS` queda vacío y las funciones `buildRutasLandingPage` / `buildRutaPage` sin uso. Las páginas de `rutas/` y `js/rutas.js` se dejan en disco, sin enlazar, por si hay enlaces entrantes. El Simulador de Sesgos ya no existía en el código; se limpió su rastro en `CLAUDE.md`.
...
"Fuera de Bata" tiene páginas estáticas hechas a mano (no salen de `generate-pages.js`, no se sobreescriben en deploy): `/fuera-de-bata/index.html` (landing) y las 10 piezas en `/fuera-de-bata/<slug>/index.html`. Ver la sección "Fuera de Bata movido a páginas propias" más arriba.
